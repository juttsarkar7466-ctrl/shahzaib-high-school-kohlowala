...
const MONGO_URI = process.env.MONGO_URI || "mongodb+srv://cheeta_db_user:Bea0N89rCALt17Oz@cluster0.z3upua5.mongodb.net/schoolDB?retryWrites=true&w=majority";
...
async function initAdmin() {
    try {
        ...
        if (count === 0) {
            await Creds.create({ 
                email: process.env.ADMIN_EMAIL || "juttsarkar7466@gmail.com", 
                password: process.env.ADMIN_PASSWORD || "JuttSSMarket@2026!" 
            });
...
app.use(session({
    secret: process.env.SESSION_SECRET || 'JuttSarkarSecretKey2026!@#',
    resave: false,
    saveUninitialized: false,
    cookie: { maxAge: 30 * 60 * 1000 } // 30 Mins Timeout
}));
